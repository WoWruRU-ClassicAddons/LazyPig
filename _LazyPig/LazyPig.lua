...
function LazyPig_OnLoad()
	...
	if(DEFAULT_CHAT_FRAME) then	DEFAULT_CHAT_FRAME:AddMessage(LP_LOADED) end
end
...
function LazyPig_Command(cmd)
	if(cmd ~= "") then
		for i=1,10 do
			if i == 10 then
				DEFAULT_CHAT_FRAME:AddMessage(LP_SYNTAX_INCORRECT)
...
function LazyPigSplit_Command(cmd)
	for i=1,100 do
		if not LPCONFIG.SPLIT then
			DEFAULT_CHAT_FRAME:AddMessage("|cffff0000"..LP_SPLIT_1)
			break		
		elseif i == 100 then
			DEFAULT_CHAT_FRAME:AddMessage("|cffff0000"..LP_SPLIT_2)
		elseif tonumber(cmd) == i then
			DEFAULT_CHAT_FRAME:AddMessage(LP_SPLIT_3..i)
			LPSPLIT.VALUE = i
			break			
		end
	end
...
function LazyPig_ReturnOptionState(i)
	...
	or i == 9 and LPCONFIG.EPLATE then return LP_ON
	else return LP_OFF
	end	
end

function LazyPig_StatusList()
	DEFAULT_CHAT_FRAME:AddMessage(LP_STATUS_MSG_0);
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."1".."|cffffffff"..LP_STATUS_MSG_1..LazyPig_ReturnOptionState(1));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."2".."|cffffffff"..LP_STATUS_MSG_2..LazyPig_ReturnOptionState(2));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."3".."|cffffffff"..LP_STATUS_MSG_3..LazyPig_ReturnOptionState(3));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."4".."|cffffffff"..LP_STATUS_MSG_4..LazyPig_ReturnOptionState(4));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."5".."|cffffffff"..LP_STATUS_MSG_5..LazyPig_ReturnOptionState(5));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."6".."|cffffffff"..LP_STATUS_MSG_6..LazyPig_ReturnOptionState(6));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."7".."|cffffffff"..LP_STATUS_MSG_7..LazyPig_ReturnOptionState(7));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."8".."|cffffffff"..LP_STATUS_MSG_8..LazyPig_ReturnOptionState(8));
	DEFAULT_CHAT_FRAME:AddMessage("> |cff00ff00".."9".."|cffffffff"..LP_STATUS_MSG_9..LazyPig_ReturnOptionState(9));
end
...
function LazyPig_OnUpdate()
	...
	if player_invite_start and (GetTime() + 0.5) > player_invite_start then
		...
		UIErrorsFrame:AddMessage(LP_AUTO_ACCEPT)
	end	
end
...
function LazyPig_OnEvent(event)	  
	...
	elseif(event == "CHAT_MSG_LOOT") then
		if((string.find(arg1 ,LP_YOU_WON) or string.find(arg1 ,LP_YOU_RECEIVE)) and (string.find(arg1 ,"cffa335e") or string.find(arg1, "cff0070d") and not string.find(arg1 ,"Bijou") and not string.find(arg1 ,"Idol") and not string.find(arg1 ,"Shard"))) then
			save_time = GetTime()
		end
...
function LazyPig_RecordQuest(questindex)
	...
	if isshift and questindex then
		if QuestRecord["qindex"] ~= questindex or QuestRecord["npc"] ~= npc then
			UIErrorsFrame:AddMessage(LP_RECORD_COMPLETE)
			QuestRecord["npc"] = npc
			QuestRecord["qindex"] = questindex
		end	
	elseif not isshift and QuestRecord["qindex"] then
		UIErrorsFrame:AddMessage(LP_RECORD_RESET)
		QuestRecord["npc"] = nil
...
function LazyPig_ReplyQuest(event, complete)
	if IsShiftKeyDown() and QuestRecord["npc"] == GetUnitName("target") and QuestRecord["qindex"] then
		...
		elseif complete and QuestRecord["complete"] then
			UIErrorsFrame:AddMessage(LP_REPLAY_COMPLETE)
...
function LazyPig_AutoSummon()
	if LPCONFIG.SUMM then
		...
		if(not player_summon_message and (expireTime ~= 0)) then
			...
			DEFAULT_CHAT_FRAME:AddMessage(LP_AUTO_SUMMON..expireTime..S);
...
function Check_Bg_Status()
	...
	for i=1, MAX_BATTLEFIELD_QUEUES do
		...
		elseif((status == "active") and not (mapName == EASTERN_KINGDOMS) and not (mapName == KALIMDOR)) then
...
function LazyPig_AutoJoinBG(index, map_name)	
	if LPCONFIG.BG then	
		...
		if(not player_bg_message and (expireTime > 3)) then
			...
			DEFAULT_CHAT_FRAME:AddMessage(LP_AUTO_JOIN.. map_name..IN..expireTime..S, 1.0, 1.0, 0.0)
...
function LazyPig_AutoLeaveBG()
	if LPCONFIG.BG then
		...
		if(bg_winner ~= nil) then	
			...
			UIErrorsFrame:AddMessage(winner_name..LP_WINS)
			LeaveBattlefield();
		end	
	end
end
...
function SaveData()
	...
	UIErrorsFrame:AddMessage(LP_DATA_SAVED)
end
